...
%
%     [W,BESTARG] = DD_GRIDSEARCH(A,CLNAME,NRFOLDS,ARG1,ARG2,...)
%
% INPUT
%   A             One-class dataset
%   CLNAME        String containing the classifier to train
%   NRFOLDS       Nr. folds in the crossvalidation (default = 10)
%   ARG1,ARG2,... Vector with possible values of input arguments
%
% OUTPUT
%   W             Trained classifier
%   BESTARG       Optimal parameter values
%
% DESCRIPTION
...
% Area under the ROC curve (AUC) performance measure (dd_auc). The
% classifier is trained using the optimized BESTARG on all training data
% A.
% An example would be:
...
if isempty(a)
	w = prmapping(mfilename,{clname,nrfolds,varargin{:}});
...
for i = 1:nrcomb
	...
	for j=1:nrfolds
        ...
        x = target_class(x);                %% vincent adds this part for experiment purpose (train only with target points)
